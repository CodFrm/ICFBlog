...
class db {
	...
	protected function parseValue($value) {
		if (is_string ( $value )) {
			$value = strpos ( $value, ':' ) === 0 && in_array ( $value, array_keys ( $this->bind ) ) ? $this->sqlHandle ( $value ) : '\'' . $this->sqlHandle ( $value ) . '\'';
	...
	function update($data,$where=0) {
		...
		if(is_string($data)){
			...
			foreach ($data as $key=>$value) {
				$sql.=($add++==1?',':'').$key.'='.$this->parseValue($value);
			}
		}
	 	if (is_string ( $where )) {
			$sql .= $where;
	 ...
	 */
	function select($where=0,$field=0) { // 我竟然写得这么麻烦....迟早有一天我要改掉 我来改啦 2017/1/5 时隔一年?233
